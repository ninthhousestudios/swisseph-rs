...
fn gauquelin_riseset() {
    let eph = Ephemeris::new(EphemerisConfig::default()).unwrap();
    let eph_star = Ephemeris::new(EphemerisConfig {
    ...
    assert_eq!(
        ...
        78,
        "expected 78 golden cases (72 planet + 6 star)"
    );
    for (i, c) in data.gauquelin_riseset.iter().enumerate() {
        let body = swisseph::types::Body::try_from(c.ipl)
            .unwrap_or_else(|e| panic!("case {i}: unknown body {}: {e}", c.ipl));
        let starname = c.starname.as_deref();
        let label = format!(
            "case {i} (ipl={} imeth={} star={:?} tjd_ut={:.6} geolon={:.6} geolat={:.6})",
            c.ipl, c.imeth, starname, c.tjd_ut, c.geolon, c.geolat
        );
        let geopos = [c.geolon, c.geolat, 0.0];
        let e = if starname.is_some() { &eph_star } else { &eph };
        let result = e.gauquelin_sector(
            c.tjd_ut,
            body,
            starname,
            CalcFlags::empty(),
        ...
        if c.retval < 0 {
            ...
            // Star rise/set cases have a wider tolerance: rise_trans's star path
            // lacks per-call TOPOCTR support (codebase-map riseset note).
            let eps = if starname.is_some() && c.imeth >= 2 {
                1e-5
            } else {
                1e-6
            };
            super::assert_f64_eps(&format!("{label} dgsect"), c.dgsect, dgsect, eps);
        }
    }
}
