...
fn golden_fixstar_positions() {
    ...
    let expected_count = 7 * 4 * 7; // NSTARS × NEPOCHS × NFLAGS
    assert_eq!(
        data.fixstar.len(),
        expected_count,
        "unexpected number of fixstar cases"
    );

    for (i, c) in data.fixstar.iter().enumerate() {
        ...
        // Verify canonical star name matches C output
        if name != c.star_out {
        ...
        for k in 0..6 {
            ...
            // Skip speeds if not requested
            if k >= 3 && !has_speed {
                continue;
            }

            let (eps, kind) = if is_xyz {
                // Cartesian AU: absolute tolerance; relative is ~1e-7 for all tested stars
                (1e-3, "AU")
            } else if k == 2 {
                // Distance: relative tolerance (parallax-less stars are ~1e9 AU)
                let rel = expected.abs() * 1e-6;
                (rel.max(1e-6), "AU_rel")
            } else if k == 5 {
                // Distance speed (AU/day): wide tolerance — C source says "speed is
                // incorrect !!!" and our stateless deflection adds a small additional
                // error that scales with star distance. Not meaningful to tighter than ~0.1.
                (0.1, "AU/day_dist")
            } else if k >= 3 {
                // Angular speed components: tolerate aberration-speed dt difference
                if is_radians {
                    (1e-8, "rad/day")
                } else {
                    (1e-6, "deg/day")
