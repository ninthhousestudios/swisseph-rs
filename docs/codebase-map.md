...
│   ├── truenode.rs     — golden tests for osculating node/apogee (swisseph-rs/84: 168 cases, 2
│                          bodies {SE_TRUE_NODE, SE_OSCU_APOG} × {MOSEPH, SWIEPH} × 6 flag combos
│                          {SPEED, SPEED|EQUATORIAL, SPEED|XYZ, SPEED|NONUT, SPEED|J2000, no_speed}
│                          × the 7 gen_calc epochs; positions eps 1e-9, speeds 1e-7 EXCEPT Moshier
│                          node/apogee speed relaxed to 5e-6 for the documented C global-cache
│                          finite-difference artifact — see CLAUDE.md <stateless_tolerance> §3)
...
│   ├── truenode.json   — C-generated reference data for swe_calc SE_TRUE_NODE/SE_OSCU_APOG (168 cases)
...
└── c-gen/
    ...
    ├── gen_truenode.c  — C harness to regenerate truenode.json (swe_calc SE_TRUE_NODE/SE_OSCU_APOG, 2 bodies × {MOSEPH,SWIEPH} × 6 flags × 7 epochs; sets ephe path for SWIEPH)
